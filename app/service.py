...
class Extract(Assemble, Obtain, Apply):
    ...
    def extract_dates(self, entities):
        ...
        print(len(dates), len(weekdays), len(delays), len(sdelays))
        
        #if len(dates) != 0 and len(weekdays) !=0: self.response_content = self.process_dates(dates, weekdays)
        
        self.response_content = self.process_dates(dates, weekdays)
        self.response_content = self.process_delays(delays, sdelays)
        #if len(delays) != 0 and len(sdelays) !=0: self.response_content = self.process_delays(delays, sdelays) # list of datetime --> response_content
        print(self.response_content)
